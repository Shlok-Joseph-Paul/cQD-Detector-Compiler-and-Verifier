...
function parseDevices(source: string): ParsedEntity<Device> {
  ...
  for (const row of prepared.rows) {
    ...
    const device: Device = {
      ...
      detector_class: read.oneOf("detector_class", DETECTOR_CLASSES),
...
function parseMeasurements(source: string): ParsedEntity<Measurement> {
  ...
  for (const row of prepared.rows) {
    ...
    const measurement: Measurement = {
      ...
      responsivity_frequency_hz: read.optionalNumber(
        "responsivity_frequency_hz",
      ),
      eqe_percent: read.nullableNumber("eqe_percent"),
      eqe_frequency_hz: read.optionalNumber("eqe_frequency_hz"),
