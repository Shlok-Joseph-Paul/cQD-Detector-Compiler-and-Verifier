...
test("the checked-in CSV dataset passes validation and joins every measurement", async () => {
  ...
  assert.equal(atlas.measurements.length, 220);
  ...
  assert.equal(preprintRecords.length, 3);
  assert.ok(
    preprintRecords.every(
      ({ paper: source }) =>
        source.paper_id === "paper-00f55628795b" && !source.peer_reviewed,
    ),
  );
  const amberRecords = atlas.records.filter(
    ({ measurement: point }) => point.flag === "amber",
  );
  assert.equal(amberRecords.length, 105);
  ...
  assert.equal(unverifiedRecords.length, 89);
...
test("missing applicable frequency evidence requires unverified status", () => {
  const staleGreen = entities({
    responsivity_a_w: 0.2,
    responsivity_frequency_hz: null,
    measurement_frequency_hz: 100,
  });
  const staleResult = validateAtlasEntities(staleGreen);
  assert.equal(staleResult.valid, false);
  assert.ok(
    staleResult.issues.some(
      ({ field, code }) =>
        field === "flag" && code === "frequency_evidence_unverified",
    ),
  );

  const unverified = entities({
    responsivity_a_w: 0.2,
    responsivity_frequency_hz: null,
    measurement_frequency_hz: 100,
    flag: "unverified",
  });
  assert.equal(validateAtlasEntities(unverified).valid, true);
  assert.equal(
    applyAutomaticAmberRules(staleGreen.measurements[0]).flag,
    "unverified",
  );
});
